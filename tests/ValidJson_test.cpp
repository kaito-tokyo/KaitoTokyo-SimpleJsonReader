...
void assertEvent(std::deque<SimpleJsonReader::Event> &expectedEvents, SimpleJsonReader::Event receivedEvent);
...
void test_emptyObject()
{
    ...
    SimpleJsonReader::parseJson(std::move(jsonString), [&expectedEvents](auto event) {
        printEvent(event);
...
void printEvent(SimpleJsonReader::Event event)
{
    std::cout << "Event:" << eventTypeToString(event.type) << "\tValue:" << event.value << "\tPath:" << jsonPathToString(event.jsonPath) << std::endl;
}
