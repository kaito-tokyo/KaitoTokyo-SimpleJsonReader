...
void test_matchesPathPrefix_emptyPrefixAlwaysMatchEmptyPath() {
  ...
  throw std::runtime_error(
    "Expected an empty prefix to match an empty path");
  }

  printPostamble("matchesPathPrefix_emptyPrefixAlwaysMatchEmptyPath",
         "PASSED");
}
...
void test_matchesPathPrefix_stringPrefixCanMatchStringPath() {
  ...
  throw std::runtime_error(
    "Expected a string prefix to match a string path");
  }

  printPostamble("matchesPathPrefix_stringPrefixCanMatchStringPath",
         "PASSED");
}
...
void test_matchesPathPrefix_indexPrefixCanMatchIndexPath() {
  ...
  if (!matchesPathPrefix(event, prefix.data(), 1)) {
  throw std::runtime_error(
    "Expected an index prefix to match an index path");
  }

  printPostamble("matchesPathPrefix_indexPrefixCanMatchIndexPath",
         "PASSED");
}
...
void test_matchesPathPrefix_indexPrefixNeverMatchStringPath() {
  ...
  printPostamble("matchesPathPrefix_indexPrefixNeverMatchStringPath",
         "PASSED");
}
...
void test_matchesPathPrefix_stringPrefixNeverMatchIndexPath() {
  ...
  printPostamble("matchesPathPrefix_stringPrefixNeverMatchIndexPath",
         "PASSED");
}
...
void test_matchesPathPrefix_stringAndIndexPrefixCanMatchStringAndIndexPath() {
  ...
  std::array<JsonPathComponent, 2> prefix{
    JsonPathComponent{"child1"sv}, JsonPathComponent{std::size_t{0}}};
...
void test_matchesPathPrefix_indexAndStringPrefixCanMatchIndexAndStringPath() {
  ...
  std::array<JsonPathComponent, 2> prefix{
    JsonPathComponent{std::size_t{0}}, JsonPathComponent{"child2"sv}};
  if (!matchesPathPrefix(event, prefix.data(), 2)) {
  throw std::runtime_error(
...
void test_matchesPathPrefix_TwoComponentsPrefixNeverMatchOneComponentsPath() {
  ...
  std::array<JsonPathComponent, 2> prefix{
    JsonPathComponent{"child"sv}, JsonPathComponent{"other"sv}};
  if (matchesPathPrefix(event, prefix.data(), 2)) {
  throw std::runtime_error(
...
void test_matchesExactPath_neverMatchIfLengthMismatch() {
  ...
  std::array<JsonPathComponent, 2> prefix{
    JsonPathComponent{"child"sv}, JsonPathComponent{"grandchild"sv}};
...
void test_matchesExactPath_matchesWhenLengthAndComponentsMatch() {
  ...
  std::array<JsonPathComponent, 2> path{
    JsonPathComponent{"child1"sv}, JsonPathComponent{std::size_t{0}}};
  if (!matchesExactPath(event, path.data(), 2)) {
  throw std::runtime_error(
    "Expected matchesExactPath to return true when length and components match");
  }
...
void test_matchesExactPath_notMatchWhenLengthMatchesButComponentsNotMatch() {
  ...
  std::array<JsonPathComponent, 2> path{
    JsonPathComponent{"child1"sv}, JsonPathComponent{std::size_t{1}}};
  if (matchesExactPath(event, path.data(), 2)) {
  throw std::runtime_error(
    "Expected matchesExactPath to return false when the components mismatch");
  }
...
void test_forEachPathComponent() {
  ...
  forEachPathComponent(event, [&components](const JsonPathComponent &component) {
    components.push_back(component);
  });

  std::vector<JsonPathComponent> expected{
    JsonPathComponent{"child1"sv}, JsonPathComponent{std::size_t{2}},
    JsonPathComponent{"child3"sv}};

  if (components != expected) {
    throw std::runtime_error(
      "Expected forEachPathComponent to visit path components from root to leaf");
  }
...
int main(int argc, char* argv[]) {
  ...
  if (testName.empty() || testName == "matchesPathPrefix_emptyPrefixAlwaysMatchEmptyPath")
    test_matchesPathPrefix_emptyPrefixAlwaysMatchEmptyPath();
  if (testName.empty() || testName == "matchesPathPrefix_emptyPrefixAlwaysMatchOneComponentPath")
    test_matchesPathPrefix_emptyPrefixAlwaysMatchOneComponentPath();
  if (testName.empty() || testName == "matchesPathPrefix_emptyPrefixAlwaysMatchTwoComponentsPath")
    test_matchesPathPrefix_emptyPrefixAlwaysMatchTwoComponentsPath();
  if (testName.empty() || testName == "matchesPathPrefix_stringPrefixCanMatchStringPath")
    test_matchesPathPrefix_stringPrefixCanMatchStringPath();
  if (testName.empty() || testName == "matchesPathPrefix_indexPrefixCanMatchIndexPath")
    test_matchesPathPrefix_indexPrefixCanMatchIndexPath();
  if (testName.empty() || testName == "matchesPathPrefix_indexPrefixNeverMatchStringPath")
    test_matchesPathPrefix_indexPrefixNeverMatchStringPath();
  if (testName.empty() || testName == "matchesPathPrefix_stringPrefixNeverMatchIndexPath")
    test_matchesPathPrefix_stringPrefixNeverMatchIndexPath();
  if (testName.empty() || testName == "matchesPathPrefix_stringAndIndexPrefixCanMatchStringAndIndexPath")
    test_matchesPathPrefix_stringAndIndexPrefixCanMatchStringAndIndexPath();
  if (testName.empty() || testName == "matchesPathPrefix_indexAndStringPrefixCanMatchIndexAndStringPath")
    test_matchesPathPrefix_indexAndStringPrefixCanMatchIndexAndStringPath();
  if (testName.empty() || testName == "matchesPathPrefix_OneComponentPrefixNeverMatchEmptyPath")
    test_matchesPathPrefix_OneComponentPrefixNeverMatchEmptyPath();
  if (testName.empty() || testName == "matchesPathPrefix_TwoComponentsPrefixNeverMatchOneComponentsPath")
    test_matchesPathPrefix_TwoComponentsPrefixNeverMatchOneComponentsPath();
  if (testName.empty() || testName == "matchesPathPrefix_initializerListOverload")
    test_matchesPathPrefix_initializerList();
  if (testName.empty() || testName == "matchesExactPath_neverMatchIfLengthMismatch")
    test_matchesExactPath_neverMatchIfLengthMismatch();
  if (testName.empty() || testName == "matchesExactPath_matchesWhenLengthAndComponentsMatch")
    test_matchesExactPath_matchesWhenLengthAndComponentsMatch();
  if (testName.empty() || testName == "matchesExactPath_notMatchWhenLengthMatchesButComponentsNotMatch")
