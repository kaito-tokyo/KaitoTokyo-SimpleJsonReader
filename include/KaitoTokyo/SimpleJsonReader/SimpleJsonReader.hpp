 ...
 * @brief   Recursive Descent JSON parser for C++ that provides SAX-like interface.
 * @version 2026.03.21
...
ErrorType parseValue(std::string_view *json, EventHandler handler, JsonPath *jsonPath) noexcept;
...
ErrorType parseString(std::string_view *json, EventHandler handler, JsonPath *jsonPath) noexcept
{
	using namespace std::string_view_literals;
...
ErrorType parseKey(std::string_view *json, EventHandler handler, JsonPath *jsonPath) noexcept
{
	std::size_t pos = json->find(kSanitizedStructuralDoubleQuote, 1);
	if (pos == std::string_view::npos) {
		return ErrorType::UnexpectedEndWhileParsingStringError;
...
ErrorType parseNumber(std::string_view *json, EventHandler handler, JsonPath *jsonPath) noexcept
{
	using namespace std::string_view_literals;
...
ErrorType parseLiteral(std::string_view *json, EventHandler handler, JsonPath *jsonPath) noexcept
{
	using namespace std::string_view_literals;
	switch (json->front()) {
	case 't':
...
ErrorType parseArray(std::string_view *json, EventHandler handler, JsonPath *parentPath) noexcept
{
	using namespace std::string_view_literals;
	...
	while (true) {
		...
		if (json->empty())
			return ErrorType::UnexpectedEndWhileParsingArrayError;
		...
		if (ErrorType err = parseValue(json, handler, &elemPath); err != ErrorType::OK)
			return err;
...
ErrorType parseObject(std::string_view *json, EventHandler handler, JsonPath *parentPath)
{
	using namespace std::string_view_literals;
	...
	while (true) {
		...
		if (ErrorType err = parseKey(json, handler, &fieldPath); err != ErrorType::OK)
			return err;
		...
		if (json->front() != ':')
			return ErrorType::FieldDelimiterMissingError;
		json->remove_prefix(1);

		if (ErrorType err = parseValue(json, handler, &fieldPath); err != ErrorType::OK)
			return err;
...
ErrorType parseValue(std::string_view *json, EventHandler handler, JsonPath *jsonPath) noexcept
{
	...
	if (json->empty())
		return ErrorType::EmptyJSONError;
